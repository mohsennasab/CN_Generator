Curve Number Studio for Windows

This folder contains a ready-to-run copy of Curve Number Studio. You do not need to install Python.

How to start

1. Extract the full zip file first. Do not run the app from inside the zip preview.
2. Double-click Curve_Number_Studio.exe.
3. Your browser should open to the local app.
4. Keep the Curve Number Studio window open while you use the app.
...
- Every run saves its GIS outputs, statistics tables, and a model run log to
  a Results folder next to Curve_Number_Studio.exe. The report and map are
...
- If the app does not start, move the folder to a simple path such as C:\Curve_Number_Studio and try again.

License

Curve Number Studio is free for personal use. For commercial use, training, workshops, or videos, contact Mohsen Tahmasebi Nasab at https://www.hydromohsen.com/.
